...
- During cold start, until the first source list has loaded, third-party notifications are held as framework objects only (nothing is read from them; at most 256, the oldest evicted first); an eviction, a vault that does not open within 15 s, or a loss the locked vault could not record at the time is written as a bounded `COLD_START` gap once the vault can be written (audit #13).
