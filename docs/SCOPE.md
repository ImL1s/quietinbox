...
| Distribution: paid Google Play + free GitHub Releases, same binary (ADR-0006) | Done (Play: 0.1.0 published 2026-09-06, 0.1.2 in review; GitHub: 0.1.2) | `versionCode` 6 / 0.1.2 (three more languages, review rounds 13–25) is the GitHub release `v0.1.2` (tag at `f6afecd`, signed APK + `SHA256SUMS.txt`) and the Play production release submitted on 2026-09-06 (Google approved and published the 0.1.0 submission the same evening; the 0.1.2 release and the zh-CN / ja-JP / ko-KR listings are still in review and roll out once approved) from the same CI-built AAB with `gplay` (one edit: bundle, listings in en-US / zh-TW / zh-CN / ja-JP / ko-KR, screenshots, release notes; it replaced the 0.1.0 release that was still in review). `versionCode` 5 / 0.1.1 (the audit fixes, issues #1–#16) was a GitHub-only release (`v0.1.1`, tag at `c6b6645`); `versionCode` 4 / 0.1.0 was the first Play submission (paid, 172 countries, no INTERNET permission, no Play Billing). A tag → `release.yml` builds the signed APK + `SHA256SUMS.txt` and runs the permission gate; store texts and images live in `fastlane/metadata/android/`, screenshots reference in `docs/screenshots/` |
