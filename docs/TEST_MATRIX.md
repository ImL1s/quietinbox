...
| Instrumented storage | Real SQLCipher + Room migrations | `VaultRoundTripTest` (journal → commit → search → suppression → reopen with persisted key; deleted conversation does not resurrect on replay), `MigrationTest` (1→2 and 2→3 against exported schemas), `DemoDataTest` (debug demo seed → counts and projection → idempotent re-seed → clear leaves nothing), `DeletionGraphTest` (5: journal payload cleared on leaving PENDING; deleting the newest message rebuilds the projection; expired copies hidden before retention and projection rebuilt after it; removing a source with data leaves nothing behind; delete-everything verified and no cached cipher outlives the old key), `SearchPagingTest` (2: 250 false-positive candidates neither hide a true hit nor under-fill a page, pages resume by cursor without overlap; a deletion token suppresses a replay of the same post but not a later post with the same text) — 15 tests | `./gradlew :platform:storage:connectedDebugAndroidTest` |
| Instrumented backup | Export → wipe → import on a real vault | `BackupRoundTripTest` (1: the backup holds only visible copies, reports media it could not read, restore rebuilds the projection and the media file decrypts under the current key) | `./gradlew :platform:backup:connectedDebugAndroidTest` |
