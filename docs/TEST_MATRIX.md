...
| Instrumented storage | Real SQLCipher + Room migrations | `VaultRoundTripTest` (journal → commit → search → suppression → reopen with persisted key; deleted conversation does not resurrect on replay), `MigrationTest` (1→2 and 2→3 against exported schemas), `DemoDataTest` (debug demo seed → counts and projection → idempotent re-seed → clear leaves nothing), `DeletionGraphTest` (5: journal payload cleared on leaving PENDING; deleting the newest message rebuilds the projection; expired copies hidden before retention and projection rebuilt after it; removing a source with data leaves nothing behind; delete-everything verified and no cached cipher outlives the old key) — 13 tests | `./gradlew :platform:storage:connectedDebugAndroidTest` |
...
| Capture coordinator | Commit fence and cold start with mocked repositories | `CaptureCoordinatorTest` (16 tests: pause discards queued events, resume rotates generation and session, non-source packages dropped after the source list loads, cancellation propagates; a source disabled while an event waits for the lock is never journaled, a pause between acceptance and commit leaves the event pending, replay is held while paused and runs on resume, replay discards a row whose source was disabled, a maintenance run drops the queue and records an exact gap) | `./gradlew :platform:capture:testDebugUnitTest` |
| Maintenance gate | Ordering of vault work against reset/restore, real coroutines | `VaultMaintenanceTest` (4 tests: work runs and returns; work refused while exclusive is active; exclusive cancels and joins work in flight; exclusive runs and pipeline-lock holders are serialised) | `./gradlew :platform:storage:testDebugUnitTest` |
